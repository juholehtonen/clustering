...
We anyway try next to cluster the full year 2000 data to see if
...
The analysis for one year's data followed the same steps as for 
the manually annotated data except we couldn't calculate adjusted
Rand index because of the lacking ground truth.
% Vectorising
Vectorising the year 2000 data, $10456$ publications, with term 
frequency cut-off values $min_df=2$
(number of occurences) and $max_df=0.1$ (portion of documents with
the term) and maximum number of terms $10000$ resulted $68541$ 
ignored terms and left the feature space of $10000$ most frequent 
terms below the $max_df=0.1$ threshold. 
...
% 251, 254, 257, 260\}$
This feature space was then reduced with LSA to $800$ components 
which resulted $47 \%$ of the explained variance of the data to be
retained.
The data was then clustered using Ward's method with selected 
values of number of clusters in a range $k=[2,260]$ to find the 
optimal value for $k$.
...
We choose this as an educated guess of the order of magnitude for
