...
publication meta data from year 2000. Each record describes some 
...
different written forms a term. We might have e.g. ``visual''
and ``visually'' or ``dog'' and ``dogs'' and we want them as 
``visual'' and ``dog'' only. This is desirable to reduce the 
redundant repetition of the data and also to reduce the dimensionality of 
...
distances at each step of merging clusters i.e. if finding the 
